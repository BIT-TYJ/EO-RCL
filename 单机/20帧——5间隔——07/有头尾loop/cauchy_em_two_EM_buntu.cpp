...
typedef Eigen::Matrix< int, Eigen::Dynamic, 3, Eigen::RowMajor > Semantic_lable_read;   //元素为：语义一致的标签位、语义标签1、语义标签2
...
public:
  ...
  std::vector< std::vector<double> > L_;     //单个loop正确的概率值(语义)
  double sum_L_;   //用于检测 first EM 收敛性
  std::vector< std::vector<double> > L_P_;    //所有odometry、loop中每个点对正确的概率
  double sum_L_P_;   //用于检测 second EM收敛性
  std::vector< std::vector<int> > L_vector;     //单个loop是否正确(根据相同向量对的个数)：0表示不正确，1表示暂时正确
  std::vector< std::vector<int> > vector_true;         //根据向量对是否相同：所有odometry、loop中每个点对是否正确: 0表示不正确，1表示正确
  std::vector< std::vector<double> > point_true_num;      //每个loop和odometry，统计其200点中 向量一致和点概率大于0.8的点数，用于二次EM各点的概率系数。
...
// Expectation steps
  ...
  // }
    ...
    for (int i = 0; i < NumLoopClosureConstraints(); i ++) {
      ...
      if(L_vector[id1][id2]==0) continue;   //根据向量对一致的数量判断 一个loop是否为正确的loop  不正确的loop不参与优化
      // std::cout << id1 << " ? " << id2 <<std::endl;
      ...
      for (std::size_t j = 0; j < match.pairs_.size(); j++) {
          ...
          if(vector_true[i][j]==0 ) continue;     //向量对不一致的匹配对 直接不参与 正确概率计算 和 优化
  ...
  void Maximization() {
    ...
    for (int i = 0; i < NumOdometryConstraints(); i++) 
    {
      ...
      {
        std::cout << id1<<" "<<id2<< std::endl;
        std::cout  <<camera_poses_.data_[id1].transformation_se3_.matrix()<<std::endl;
      }
        ...
        if(1)       //id1 !=120  ///////////////////////////////////////////////////////////
        {
          for (std::size_t j = 0; j < match.pairs_.size() && j<200; j++) 
          {
            ...
            if(match.semantic_label[j]==1 && vector_true[i+NumLoopClosureConstraints()][j]==1)          //odometry没有进行语义筛除  向量判断的不正确的点不参与优化
            {

              if(id1==157)
              {
                std::cout << match.pairs_[j].first  <<"\n" << match.pairs_[j].second << std:: endl;
                std:: cout << "  " <<std::endl;
              }
...
void Vector()       //计算一个loop中正确向量的个数
      ...
      std::cout << id1 << "   " << id2 <<" ";
        if(1)         //id1 !=120  ///////////////////////////////////////////////////////////
        {
          ...
          for (std::size_t k = j+1; k < match.pairs_.size() && k<200; k++) 
          {
              ...
              Eigen::Vector3d vec_d = (match.pairs_[k].first - match.pairs_[j].first)-(match.pairs_[k].second - match.pairs_[j].second);
              if(vec_d[0]<0.1 && vec_d[0]>-0.1 && vec_d[1]<0.1 && vec_d[1]>-0.1 && vec_d[2]<0.1 && vec_d[2]>-0.1)    
              {
                l++;
  ...
  {            
      ...
      // std::cout << id1 << "   " << id2 <<std::endl;
        if(1)       //id1 !=120  ///////////////////////////////////////////////////////////
        {
          for (std::size_t j = 0; j < match.pairs_.size() && j<200; j++) 
          {
              ...
              //   std::cout << "correct probability of point "<<j <<" is: "<< L_P_[i+NumLoopClosureConstraints()][j]<< "  ";
    ...
    //测试     统计loop\odometry 中， 200个点中向量一致且点概率>0.8的点数，用于二次EM的点概率系数。1068-1118
    // int num_vect_true=0;
    // for (int i = 0; i < NumOdometryConstraints(); i++) 
    // {
    ...
    //   //  std::cout << id1 << "   " << id2 <<std::endl;
    //       for (std::size_t k = 0; k < match.pairs_.size() && k<200; k++) 
    //       {
    //           if(vector_true[i+NumLoopClosureConstraints()][k]==0)   continue;
    //           if(L_P_[i+NumLoopClosureConstraints()][k]>0.8)    
    //           {
    //             num_vect_true++;
    //           }
    //       }
    //   // std::cout<<num_vect_true<<"\n" <<std::endl;
    //   if(num_vect_true!=0) point_true_num[id1][id2]=200/num_vect_true;           
    // }
...
void Maximization_point() {
    ...
    for (int i = 0; i < NumOdometryConstraints(); i++) 
    {
      ...
      // if(point_true_num[id1][id2]==0)  continue;         //如果一个odometry的向量一致且概率大于0.8的点数为0， 跳过
        ...
        if(1)       //id1 !=120  ///////////////////////////////////////////////////////////
        {
          for (std::size_t j = 0; j < match.pairs_.size() && j<200; j++) 
          {
            ...
            //  if(L_P_[i+NumLoopClosureConstraints()][j]<0.8) continue;      //点概率<0.8的直接跳过
            // else L_P_[i+NumLoopClosureConstraints()][j]=1;

            if(match.semantic_label[j]==1 && vector_true[i+NumLoopClosureConstraints()][j]==1)          //语义筛除+向量筛除
            {
            ceres::CostFunction* cost_odometry =
                new ceres::AutoDiffCostFunction<CauchyFunctor, 3,
                                              Sophus::SE3d::num_parameters,
    ...
    for (int i = 0; i < NumLoopClosureConstraints(); i++) 
    {
      ...
      // if(point_true_num[id1][id2]==0)  continue;         //如果一个loop的向量一致且概率大于0.8的点数为0， 跳过
      //  if(L_[id1][id2] < TRUST_ODOMETRY*0.2)  continue;          //loop正确概率  不进行outlier loop的优化
       if(L_vector[id1][id2]==0) continue;    //向量判断   错误loop直接不参与优化

        for (std::size_t j = 0; j < match.pairs_.size(); j++) 
        {

              //  if(L_P_[i][j]<0.8) continue;            //点概率<0.8的直接跳过
              //  else L_P_[i][j]=1;

          if(match.semantic_label[j]==1 && vector_true[i][j]==1)   //语义+向量
          {
          // {
          ceres::CostFunction* cost_loop =
              new ceres::AutoDiffCostFunction<CauchyFunctor, 3,
  ...
  bool IsConverged_second() {
    ...
    if (fabs(lambda_second_ - last_lambda_second_) > 0.001) {
      std::cout << "lambda was updated" << std::endl;
      return false;
    }
...
int main(int argc, char** argv) {
  ...
  r2em_c.L_P_.resize(r2em_c.NumLoopClosureConstraints()+r2em_c.NumOdometryConstraints());
    for (int i = 0; i < r2em_c.NumLoopClosureConstraints()+r2em_c.NumOdometryConstraints(); i++) {
      r2em_c.L_P_[i].resize(200);                    //每个loop的点对数为50
    }
  ...
  r2em_c.vector_true.resize(r2em_c.NumLoopClosureConstraints()+r2em_c.NumOdometryConstraints());
    for (int i = 0; i < r2em_c.NumLoopClosureConstraints()+r2em_c.NumOdometryConstraints(); i++) {
      r2em_c.vector_true[i].resize(200);                    //每个loop的点对数为50
    }

  r2em_c.point_true_num.resize(r2em_c.NumPoses());         //统计loop\odometry 中， 200个点中向量一致且点概率>0.8的点数，用于二次EM的点概率系数。
    for (int i = 0; i < r2em_c.NumPoses(); i++) {
      r2em_c.point_true_num[i].resize(r2em_c.NumPoses());
    }
  ...
  std::cout << "semantic probability matrix" << std::endl;
    ...
    for (int i = 0 ; i < NUM_ITERATION_EM; i ++) {
      ...
      r2em_c.Maximization();
        ...
        if (i == 0)
         r2em_c.SaveCameraPoses("final_poses_em_0nd.txt");
    ...
    for (int i = 0 ; i < NUM_ITERATION_EM; i ++) {
      ...
      // if (i == 30)
      //    r2em_c.SaveCameraPoses("final_poses_em_30nd.txt");
