      ...
      int id2 = match.id2_;
        ...
        { 
          ...
          for (std::size_t j = 0; j < match.pairs_.size() && j<200; j++) 
          {
            ...
            if(vector_true[i+NumLoopClosureConstraints()][j]==0)          //odometry没有进行语义筛除  向量判断的不正确的点不参与优化
            {
            ...
            if(vector_true[i+NumLoopClosureConstraints()][j]==1)          //odometry没有进行语义筛除  向量判断的不正确的点不参与优化
            {
